		...
		try {
			...
			if (isReadyPDDList()) {
				...
				for (int i = 0; i < list.getDeviceCount (); i++)
				{
					 ...
					 */  
					//PRINTS THE PORT e.g. 6665 + list.getDevList ()[i].getRobot ()
					//PRINTS INTERFACE e.g. sonar + pcu.lookupName (list.getDevList ()[i].getInterf ()
					//PRINTS Index of interface  + list.getDevList ()[i].getIndex ()

					playerInfo.append ( pcu.lookupName (list.getDevList ()[i].getInterf ()) +
							":" + list.getDevList ()[i].getIndex ());
					PlayerDevAddr pda = list.getDevList ()[i];

					provides.add(i,pcu.lookupName (list.getDevList ()[i].getInterf ()) );
					index.add(i,list.getDevList ()[i].getIndex () );
					...
					if (isReadyPDDI ())
					{
						...
						if(toggle)
						{
							name=(getPDDI ().getDriverName());
							System.out.println("in playerclient"+name);
							toggle=false;
						}
					}
				}
