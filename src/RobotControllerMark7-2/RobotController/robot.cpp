...
Robot::Robot(PlayerClient& pc)
:mPlayerClient(pc), mIOService(), mSocket(mIOService)
{
...
void Robot::do_state_action_ack()
{
	...
	string cmd;
	long session_id;
	if (read(ss) && (ss >> cmd) && (cmd.find(CMD_ACK) != string::npos) && (ss >> session_id)) {
		if (session_id < 0) {
			mSessionID = -1;
...
void Robot::do_state_action_pose()
{
	...
	try {
		xp = mPosition2D->GetXSpeed();
		yp = mPosition2D->GetYSpeed();
		ap = mPosition2D->GetYawSpeed();
	...
	stringstream oss;
	oss << CMD_POSE << " " << xp << " " << yp << " " << ap << " " << confidence;
	if (write(oss)) {
		cerr << signature << " - success; next state: STATE_IDLE" << endl;
	} else {
		cerr << signature << " - failure; next state: STATE_IDLE" << endl;
